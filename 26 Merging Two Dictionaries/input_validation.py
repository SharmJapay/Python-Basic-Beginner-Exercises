"""
String Manipulation and Input Validation Utility Module.

This module provides a collection of robust, user-interactive functions designed
to safely collect validated inputs (strings, positive integers, and specific keyword choices)
from the console, alongside a core string-trimming utility function.
"""


def input_dictionary() -> dict:
    """Returns a dictionary data collection

    Returns
        data [dict]: Dictionary data collection
    """

    while True:
        try:
            collect_data = input(
                '\nEnter a dictionary (keys: values separated by comma) (e.g  "name": "Alice", "age": 25): '
            )

            if not collect_data:
                raise ValueError

            dictionary = {}

            try:
                for item in collect_data.split(","):
                    if not item.strip():
                        continue

                    key, colon, value = item.partition(":")

                    if not colon:
                        raise ValueError

                    clean_key = key.strip().strip("\"'")
                    clean_value = value.strip().strip("\"'")

                    if not clean_key:
                        raise ValueError

                    if clean_value.isdigit():
                        clean_value = int(clean_value)

                    dictionary[clean_key] = clean_value

                return dictionary

            except ValueError:
                print("Error! Wrong format. Try Again.")
                continue

        except ValueError:
            print("Error! Cannot accept empty input value. Try Again.")


def confirm_exit() -> str:
    """Returns 'yes' or 'no' string value

    Returns
        text [str]: The 'yes' or 'no' string value
    """

    while True:
        try:
            answer = input("\nDo you want to close the program? (Type 'Yes' or 'No'): ")

            if answer.lower() == "yes" or answer.lower() == "no":
                return answer

            raise ValueError

        except ValueError:
            print("Error! Accepts 'Yes' or 'No' values only. Try Again.")
